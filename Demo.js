	...
	window.demo = { 
		...
		'init': function() {
			...
			Modernizr.load({ 'test': ( location.href.indexOf(this.url) > -1 ), 'yep': 'js/ga.js' });
			this.test('Backbone', function() {
				...
				ForumCollection = Backbone.Collection.extend({ 'url': {"responseData": null, "responseDetails": "bad or missing callback or context", "responseStatus": 400}.format(encodeURIComponent(demo.forum)), 'parse': function(response) { return response.responseData.feed.entries; } });
